...
grep -q 'privilege>http://developer.samsung.com/privilege/avplay' "$TIZEN_MANIFEST" \
  || fail "tizen-manifest missing avplay privilege"
...
for f in \
  ...
  packages/prairie_tizen/lib/platform/avplay_video_backend.dart \
...
grep -q 'video_player_avplay:' "${ROOT}/packages/prairie_tizen/pubspec.yaml" \
  || fail "prairie_tizen missing video_player_avplay"
